...
def _held_assets() -> list[str]:
    """Assets CURRENTLY held (shares > 0 today). A fully-sold asset keeps its
    ledger + realized P&L but must vanish from the holdings views (correlation,
    attribution, per-asset detail, PDF positions)."""
    try:
        cur = data.current_shares()
        return [a for a in data.ASSETS if cur.get(a, 0) > 1e-9]
    except Exception:
...
def headline_stats(price_history: pd.DataFrame) -> dict[str, float | None]:
    """Annualized return + max drawdown, for the Dashboard KPI strip.

    Lives here, beside the Risk Metrics tab, so both screens share ONE
    implementation: same series (the unitized VL, so flows aren't mistaken for
    performance), same formulas — EAR on the mean daily return, drawdown off
    the running peak. Duplicating them in app.py would let the Dashboard and
    the Analytics tab drift apart.

    Returns None for a field rather than 0.0 when there isn't enough history,
    so the caller can render "—" instead of a confident wrong number.
    """
    out: dict[str, float | None] = {"ann_return": None, "max_dd": None}
    try:
        vl = _portfolio_vl_series(price_history)
    except Exception:
        return out
    if vl is None or len(vl) < 2:
        return out
    rets = _daily_returns(vl)
    if rets is not None and len(rets) > 0:
        mean_d = rets.mean()
        if pd.notna(mean_d):
            out["ann_return"] = float((1.0 + mean_d) ** TRADING_DAYS - 1.0)
    dd = vl / vl.cummax() - 1.0
    if not dd.empty and pd.notna(dd.min()):
...
def _section(palette: dict, title: str, subtitle: str = "") -> None:
    # Route through the shared theme markup so every Analytics section renders
    # as the same panel-header bar the rest of the app uses (page CSS vars are
    # already injected by app.build_css). Keeps Analytics visually in step with
    # the Dashboard instead of its own ad-hoc header style.
    import theme as _va1theme
    st.markdown(_va1theme.section_head(title, subtitle or None),
...
def _render_risk_metrics(static, price_history: pd.DataFrame, palette: dict) -> None:
    ...
    if n < 20:
        st.warning(
            f"Only {n} days of returns available. The **annualized** figures "
...
def _render_calendar_heatmap(static, price_history: pd.DataFrame, palette: dict) -> None:
    ...
    # Monthly return = last business day of the month vs last business day of the
    # PREVIOUS month. A month is computed ONLY when BOTH real month-end closes
    # exist in the data: resample("ME").last() takes each month's last available
    # close, and pct_change() requires the previous month-end too — so the
    # inception month (no real prior month-end) is correctly dropped, never
    # fabricated from a synthetic base-100 anchor.
    ...
    # Year-level summary — the TRUE calendar-year return, read close-to-close off
    # the VL history, NOT the product of the heatmap's monthly cells.
    #
    # Compounding the monthly cells understates the year: the heatmap drops the
    # inception month (no prior month-end to price against) and the current,
    # still-open month (no month-end yet), so a year that started or is running
    # mid-month loses that stub. Here each year is last VL of the year over the
    # last VL of the PREVIOUS year (the inception base for the first year) — so
    # January counts, the partial first/last months count, and the current year
    # is the real YTD.
    years = sorted({int(y) for y in nav.index.year}, reverse=True)
    yearly_rows: list[tuple[int, float, bool]] = []
    this_year = dt.date.today().year
    for y in years:
        in_year = nav[nav.index.year == y]
        if in_year.empty:
            continue
        end_val = float(in_year.iloc[-1])
        prev = nav[nav.index.year == (y - 1)]
        base = float(prev.iloc[-1]) if len(prev) else float(in_year.iloc[0])
        if base and base > 0:
            yearly_rows.append((y, end_val / base - 1.0, y == this_year))

    if yearly_rows:
        _section(palette, "Yearly Returns",
                 "Calendar-year performance from the unit-value history "
                 "(close-to-close). The current year is year-to-date.")
        cols = st.columns(min(len(yearly_rows), 6))
        for i, (year, val, is_ytd) in enumerate(yearly_rows):
            tone = "up" if val >= 0 else "down"
            label = f"{year} · YTD" if is_ytd else str(year)
            cols[i % len(cols)].markdown(
                _kpi(label, f"{val*100:+.2f} %", tone=tone, palette=palette),
                unsafe_allow_html=True,
            )
...
def _render_correlation(static, price_history: pd.DataFrame, snapshot: dict, palette: dict) -> None:
    ...
    _held = _held_assets() or list(data.ASSETS)
    # Start at the effective inception (Settings date, else 1st transaction).
    try:
        _inc = data.get_inception_date()
        price_history = price_history[price_history["date"].dt.date >= _inc]
    except Exception:
        pass
    df = price_history.sort_values("date").set_index("date")[_held]
    rets = df.pct_change()
    ...
    corr_colorscale = [
        [0.0,  "#B3261E"],   # z=-1.0 → negative
        [0.25, "#D98E7A"],   # z=-0.5
        [0.5,  "#E8D9A8"],   # z= 0.0 → neutral
        [0.75, "#7FB9A6"],   # z=+0.5
        [1.0,  "#0F766E"],   # z=+1.0 → positive
    ]
    fig = go.Figure(data=go.Heatmap(
        z=corr.values,
        x=list(corr.columns),
        ...
        text=text,
        texttemplate="%{text}",
        textfont=dict(size=13, color="#0B1220"),
...
def _render_attribution(static, price_history: pd.DataFrame, palette: dict) -> None:
    ...
    try:
        _inc = data.get_inception_date()
        hist = hist[hist["date"].dt.date >= _inc].reset_index(drop=True)
    except Exception:
        pass
    if hist.empty:
        st.info("Not enough history from the inception date.")
        return
    ...
    for a in (_held_assets() or list(data.ASSETS)):
    ...
    # Height auto-adjusts to the number of bars so it's never cramped or huge.
    _h = max(280, 44 * len(att) + 130)
    _style_fig(fig, palette, height=_h, showlegend=False)
    # X-axis in the SAME units as the bars (Contribution × 100 = percentage
    # points). Pad both ends so the outside value labels stay inside the frame.
    _vals = (att["Contribution"] * 100).tolist()
    _lo = min(min(_vals), 0.0)
    _hi = max(max(_vals), 0.0)
    _pad = max((_hi - _lo) * 0.22, 0.05)
    ...
    st.markdown(
        _va1theme.df_table(
            tbl,
            right_cols=["Start €", "End €", "Weight start", "Return", "Contribution"],
            signed_cols=["Return", "Contribution"]),
        unsafe_allow_html=True,
    )
...
def _fetch_benchmark(ticker: str, start: dt.date, end: dt.date) -> pd.Series:
    """Fetch a benchmark price series (via the isolated Yahoo worker) and cache it
    for an hour. Never imports yfinance in-process (curl_cffi crash isolation)."""
    if not prices.YAHOO_ENABLED:
        return pd.Series(dtype=float)
    series = prices.fetch_benchmark_series(ticker, start, end)
    if not series:
        return pd.Series(dtype=float)
    idx = pd.to_datetime([d for d, _ in series])
    return pd.Series([p for _, p in series], index=idx)
...
def _render_benchmark(static, price_history: pd.DataFrame, palette: dict) -> None:
    ...
    if not prices.YAHOO_ENABLED:
        _section(palette, "Benchmark Comparison",
                 "Compare your NAV against a reference benchmark.")
        st.info("Benchmark comparison needs live prices from Yahoo Finance, which "
                "is temporarily disabled while the data provider is stabilised.")
        return

    # Label the user's own line with the CURRENT portfolio's name — never a
    # hardcoded "GPCP" (that leaked another portfolio's name into every view).
    try:
        pf_label = (data.current_portfolio().get("name") or "").strip() or "Portfolio"
    except Exception:
        pf_label = "Portfolio"
    ...
    _CUSTOM_BM = "Custom — enter a Yahoo ticker"
    bm_choice = st.selectbox(
        "Benchmark",
        options=[*BENCHMARKS.keys(), _CUSTOM_BM],
        index=0,
        help="Choose a preset benchmark, or Custom to enter any Yahoo ticker.",
        key="bm_choice",
    )

    if bm_choice == _CUSTOM_BM:
        custom_tk = st.text_input(
            "Yahoo ticker", key="bm_custom_ticker",
            placeholder="e.g. SPY, ^GSPC, VWCE.DE, BTC-EUR",
            help="Any Yahoo Finance ticker — index (^GSPC), ETF (SPY), FX or "
                 "crypto pair. Compared on total return, indexed to 100.")
        custom_tk = (custom_tk or "").strip()
        if not custom_tk:
            st.info("Enter a Yahoo ticker to compare against.")
            return
        bm_info = {"ticker": custom_tk, "label": custom_tk.upper()}
    else:
    ...
    # Normalize BOTH indices to naive midnight before intersecting. The VL dates
    # come from the DB (they can carry a time or tz on the cloud); the benchmark
    # dates are naive midnight. Without this, the intersection silently drops
    # dates that differ only by time/tz, shrinking or shifting the window — and
    # every metric below would then run on a different (wrong) set of dates.
    ...
    # ALL stats use this single window: the intersection of the two series,
    # from the first common date to the LAST common date. Both are sliced to it
    # before any return / alpha / beta / correlation is computed.
    ...
    # The comparison can only run over dates BOTH series cover. When either the
    # benchmark's fetched history or your stored prices don't reach back to your
    # inception, the window silently starts late — and the benchmark's return
    # then looks wrong because it's measured from a later, higher starting point
    # (this is what made PSP5.PA read +12.76% instead of ~+16% from a 31-Dec
    # inception). Surface it: show the exact window, and flag a late start.
    try:
        _inception = data.get_inception_date()
    except Exception:
        _inception = None
    win_start = common.min().date()
    win_end = common.max().date()
    _bm_first = bm.index.min().date() if len(bm.index) else win_start
    _nav_first = nav.index.min().date()
    if _inception and win_start > _inception + dt.timedelta(days=4):
        # Which side is short? Blame the one whose data starts later.
        if _bm_first > _nav_first + dt.timedelta(days=4):
            culprit = (f"the benchmark's Yahoo history only starts "
                       f"{_bm_first.strftime('%d %b %Y')}")
        else:
            culprit = (f"your stored price history only starts "
                       f"{_nav_first.strftime('%d %b %Y')} — "
                       f"**Settings → Re-download ALL prices** backfills it")
        st.warning(
            f"Comparison runs from **{win_start.strftime('%d %b %Y')}**, later than "
            f"your **{_inception.strftime('%d %b %Y')}** inception, because {culprit}. "
            "Returns below are for that shorter window, so they won't match the "
            "full-period figures on the Dashboard."
        )
    ...
    fig.add_trace(go.Scatter(
        ...
        name=pf_label, hovertemplate="%{x|%d %b %Y}<br><b>%{y:.2f}</b><extra></extra>",
    ))
    fig.add_trace(go.Scatter(
    ...
    _section(palette, "Relative performance statistics",
             f"Window: {win_start.strftime('%d %b %Y')} → "
             f"{win_end.strftime('%d %b %Y')} · {len(common)} common trading days.")
    c1, c2, c3, c4 = st.columns(4)
    tone = "up" if outperf >= 0 else "down"
    c1.markdown(_kpi("Outperformance", fmt_pct(outperf),
                     delta=f"{pf_label} {fmt_pct(nav_tot)} vs BM {fmt_pct(bm_tot)}",
    ...
    _section(palette, "Cumulative outperformance (index points)",
             f"{pf_label} − Benchmark. Above zero = you beat the market, below = you underperform.")
    fig_rel = go.Figure()
...
def _render_monte_carlo(static, price_history: pd.DataFrame,
    ...
    fig.add_trace(go.Scatter(
        ...
        fill="tonexty", fillcolor="rgba(61, 111, 168, 0.16)",
        name="P10–P90 range",
...
def _load_compositions() -> dict:
    """Read etf_compositions.json — the per-ETF geo + sector breakdowns — and
    layer any user-entered manual geography on top.

    On the cloud backend the JSON file is ephemeral, so manual geo is persisted
    per-user in Postgres (`data.get_all_manual_geo`) and overlaid here so it
    survives reboots and wins over the automatic geography (sector stays auto)."""
    comps: dict = {}
    if COMPOSITIONS_PATH.exists():
        with open(COMPOSITIONS_PATH, "r", encoding="utf-8") as f:
            comps = json.load(f)
    getter = getattr(data, "get_all_manual_geo", None)
    if getter is not None:
        try:
            for asset, g in (getter() or {}).items():
                if g:
                    entry = dict(comps.get(asset) or {})
                    entry["geo"] = g
...
def _live_geo_sector(ticker: str, isin: str = ""):
    ...
    # 1b) FUND SECTOR fallback → Yahoo `funds_data.sector_weightings` (the fund's
    #     "Summary" sector split) when the ISIN factsheet + JustETF gave NO sector.
    #     Geo stays factsheet-only for funds (Yahoo has no holdings geography).
    #     Naturally a no-op for a plain stock (funds_data is empty) → step 2 below.
    if not sec and (ticker or isin) and prices.YAHOO_ENABLED:
        try:
            fb = _cs._yfinance_sector_fallback(isin, ticker=ticker)
            if fb and _cs._validate(fb, "sector")[0]:
                sec = _cs._normalize_100(fb)
        except Exception:
            pass
    ...
    if not geo and not sec and ticker and prices.YAHOO_ENABLED:
        try:
...
def _render_sector_geo(static, price_history: pd.DataFrame, snapshot: dict, palette: dict) -> None:
    ...
    if not geo_df.empty and geo_df.iloc[0]["%"] > 50:
        insights.append(
            f"High geographic concentration: **{geo_df.iloc[0]['Country / Region']}** = "
            f"**{geo_df.iloc[0]['%']:.1f} %** of the portfolio."
        )
    if not sec_df.empty and sec_df.iloc[0]["%"] > 30:
        insights.append(
            f"Sector concentration: **{sec_df.iloc[0]['Sector']}** = "
            f"**{sec_df.iloc[0]['%']:.1f} %**."
        )
    ...
    with c1:
        ...
        tbl_g["%"] = tbl_g["%"].map(lambda v: f"{v:.2f} %")
        import theme as _va1theme
        st.markdown(_va1theme.df_table(tbl_g, right_cols=["€", "%"]),
                    unsafe_allow_html=True)
    with c2:
        st.markdown(f"<div style='color:{palette['MUTED']};font-size:11px;"
        ...
        tbl_s["%"] = tbl_s["%"].map(lambda v: f"{v:.2f} %")
        import theme as _va1theme
        st.markdown(_va1theme.df_table(tbl_s, right_cols=["€", "%"]),
                    unsafe_allow_html=True)
    ...
    for asset in (_held_assets() or list(data.ASSETS)):
        ...
        with cge:
            st.markdown(_va1theme.df_table(
                geo_e_df, right_cols=["%", "Contribution portfolio"]),
                unsafe_allow_html=True)
        with cse:
            st.markdown(_va1theme.df_table(
                sec_e_df, right_cols=["%", "Contribution portfolio"]),
                unsafe_allow_html=True)
...
def _generate_pdf_report(static, price_history: pd.DataFrame, snapshot: dict, period: str):
    ...
    ACC = colors.HexColor("#0F2547")    # trust navy (accent / portfolio line)
    DARK = colors.HexColor("#0B1220")   # near-black navy ink (headers, bands)
    INK = colors.HexColor("#1E2C3F")    # primary body text
    GREY = colors.HexColor("#64748B")   # muted captions / labels
    GRID = colors.HexColor("#E2E8F0")   # hairline borders
    ZEBRA = colors.HexColor("#F7FAFC")  # table zebra
    SOFT = colors.HexColor("#F1F5F9")   # soft surface
    POS = colors.HexColor("#0F766E")    # teal-green (deutan-safe)
    NEG = colors.HexColor("#B3261E")    # crimson
    WHITE = colors.white
    ...
    meta = Table([[Paragraph(
        f"<font color='#64748B' size=8>Inception <b>{inc_str}</b> &nbsp;&nbsp;|&nbsp;&nbsp; "
    ...
    def _tile(label, value, hexcol="#1E2C3F"):
        return Paragraph(
            f"<font size=6.5 color='#64748B'><b>{label.upper()}</b></font><br/>"
            f"<font size=13.5 color='{hexcol}'><b>{value}</b></font>",
            ParagraphStyle("tile", parent=body, leading=17))

    def _sign_hex(v):
        return "#1E2C3F" if _bad(v) else ("#0F766E" if v >= 0 else "#B3261E")

    # Total return = money-weighted real gain (NAV − invested) / invested.
    # Computed here from snapshot fields so it's identical on every backend
    # (the VL / unit value is the separate time-weighted metric).
    _ni = snapshot.get("net_invested") or 0.0
    mwr = ((snapshot.get("total_value", 0.0) - _ni) / _ni) if _ni > 0 else 0.0
    tiles = [
        _tile("Net asset value", mny(snapshot.get("total_value"))),
        _tile("Total return", pct(mwr), _sign_hex(mwr)),
        _tile("1-Year", pct(pp.get("1Y")), _sign_hex(pp.get("1Y"))),
        _tile("Sharpe ratio", num(sharpe)),
        _tile("Max drawdown", pct(maxdd), "#B3261E" if not _bad(maxdd) else "#1E2C3F"),
    ]
    tw = CONTENT_W / 5.0
    ...
    kd = [
        ...
        ["Total P&L", f"{mny(snapshot.get('total_return_eur'), True)} ({pct(mwr)})",
    ...
    if not vser.empty:
        try:
            ...
            if bench_pts:
                lp.lines[1].strokeColor = colors.HexColor("#94A3B8")
            ...
            legend = "<font color='#0F2547'><b>—— Portfolio</b></font>"
            if bench_pts:
                legend += f" &nbsp;&nbsp;&nbsp; <font color='#94A3B8'><b>- - {bench_label}</b></font>"
            story.append(Paragraph(
                "<font color='#64748B' size=8>UNIT VALUE — base 100 at inception "
    ...
    det_colors = []
    for i, asset in enumerate(_held_assets() or list(data.ASSETS), start=1):
    ...
    det_rows.append(["TOTAL", "", "", f"{snapshot.get('total_value', 0):,.2f}", "100.00 %",
                     pct(snapshot.get("daily_pnl_pct")), pct(mwr)])
...
def render(static, price_history: pd.DataFrame, snapshot: dict, palette: dict) -> None:
    ...
    # Skip the (heavy: Monte Carlo numpy, correlation, benchmark/sector fetches)
    # Pro compute during Streamlit Cloud's bare-mode health-check reruns — running
    # it concurrently with a real session on the small free-tier container is what
    # segfaults a native lib. Real user runs have a ScriptRunContext.
    try:
