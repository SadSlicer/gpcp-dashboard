...
st.markdown(va1theme.build_css(st.session_state.theme, sidebar_nav=True),
...
if _DEMO:
    ...
    with _db1:
        st.markdown(
            ...
            "font-size:13px;color:var(--accent)'><b>Demo mode — sample "
            "portfolio.</b> One BlackRock ETF (iShares Core S&amp;P 500), 5 "
            "years of real adjusted-close history in EUR — shown for "
            "demonstration only, <b>not investment advice</b>. Read-only: no "
            "change is saved.</div>",
            unsafe_allow_html=True,
        )
    with _db2:
...
def kpi(label: str, value: str, delta: str | None = None,
        direction: str = "", live: bool = False, help: str = "") -> str:
    """Render a VA1 KPI card. `live=True` adds a pulsing "Live" badge.
    `help` adds a hover tooltip + a "?" cue (used to explain metrics that are
    correct but easy to misread, e.g. time-weighted vs money-weighted)."""
    return va1theme.kpi_card(label, value, delta=delta,
                              direction=direction, live=live, help=help)
...
def _auto_refresh_if_stale() -> str | None:
    """Refresh-on-connect: once per real session, run the full price pipeline
    (`daily_update.run`) — it BACKFILLS any gap in the stored history, overwrites
    the last ~7 closes with the official ones, then fetches + saves the latest.

    Why unconditional (was: only after 17:30 and only if today's close was
    missing): stale or gappy stored prices are what silently corrupt the VL,
    the benchmark window and the FX — and users hit exactly that, cured only by
    a manual Refresh. Running the same thing automatically on connect removes
    the manual step. Guarded so it runs at most ONCE per session (never on every
    rerun/click) and never in bare mode or demo. Idempotent: a no-op when the DB
    already holds the latest closes."""
    ...
    if st.session_state.get("auto_refresh_done"):
        return None                      # once per session — not on every rerun
    st.session_state.auto_refresh_done = True

    latest_before = data.latest_price_date()
    try:
        daily_update.run(force=False)
        # Any change to stored prices (new close, backfilled gap, corrected
        # row) must invalidate the per-user data cache so the page reads fresh.
        _refresh_excel_cache()
        st.session_state.pop("live_prices", None)   # force snapshot rebuild
        if data.latest_price_date() != latest_before:
            return "Prices refreshed automatically from Yahoo Finance."
        return None
...
def _invalidate_after_write() -> None:
    """Call right before st.rerun() after a write that changes the portfolio
    (a transaction, an inception change, a portfolio switch). Drops the cached
    data load and re-arms the gap-heal so every tab rebuilds its derived data
    (VL, snapshot, benchmark window…) from the freshly-written DB — no tab left
    reading pre-write / stale values. Deliberately does NOT force a full Yahoo
    refetch (that already ran on connect); the gap-heal backfills any newly
    needed history and is a cheap no-op otherwise, so edits stay responsive."""
    _refresh_excel_cache()
    st.session_state.pop("live_prices", None)
    st.session_state.pop("price_heal_done", None)     # re-run the gap heal
...
_bar_left, _bar_tx, _bar_right = st.columns([7, 1.6, 1.6],
...
with _bar_tx:
    # Transactions live here now (moved out of History): a prominent, always-
    # visible entry point in the app bar. Opens the dedicated Transactions view.
    if not _DEMO and st.button("+ Transaction", width="stretch",
                               key="__open_tx",
                               help="Add a buy, sell, deposit or withdrawal — "
                                    "and review or delete past transactions."):
        st.session_state["_show_tx"] = True
        st.rerun()
with _bar_right:
...
def render_holdings(*, show_footer: bool = True) -> None:
    ...
    # Day's FX move for the dedicated FX column: the currency component of the
    # day's change, isolated from the stock move. Since
    #   (1+pf) = (1+stock)·(1+fx)   [portfolio-ccy = stock × currency]
    # the FX-only daily return is (1+pf)/(1+stock) − 1. "—" for an asset already
    # in the portfolio currency (no FX at play).
    def _fx_daily(stock: float | None, pf: float | None,
                  native_ccy: str, target_ccy: str) -> float | None:
        if native_ccy.upper() == target_ccy.upper():
            return None
        if (stock is None or pf is None or pd.isna(stock) or pd.isna(pf)
                or (1.0 + stock) == 0):
            return None
        return (1.0 + pf) / (1.0 + stock) - 1.0
    ...
    for asset in data.ASSETS:
        ...
        rows.append({
            ...
            # % Chg is now PURE stock performance only. The FX move of the day
            # lives in its own column, so the old "+0.00% · -0.02% EUR" dual
            # cell is gone.
            "% Chg": p.get("daily_return"),
            "FX": _fx_daily(p.get("daily_return"), p.get("daily_return_pf"),
    ...
    if "Cash" in snapshot["positions"]:
        ...
        rows.append({
            ...
            "P&L %": float("nan"), "% Chg": float("nan"), "FX": None,
            "MTD %": float("nan"),
    ...
    if not pos_df.empty:
        ...
        total_row = {
            ...
            "% Chg": live_pnl_pct,
            # The TOTAL's daily move already blends stock + FX; a single "FX"
            # cell for the whole book would be ambiguous, so leave it blank.
            "FX": None,
        ...
        def _row_cells(r: dict, *, is_total: bool) -> list:
            return [
                ...
                (_pctf(r["% Chg"]), _tone(r["% Chg"]), _num(r["% Chg"])),
                (_pctf(r["FX"]), _tone(r["FX"]), _num(r["FX"])),
        ...
        if show_footer:
            ...
            # ---- Currency effect: how much of the P&L is FX, not the market ---
            # Exact additive split of each position's € P&L (convention: FX
            # applies to the invested capital):
            #   market € = shares · (price − cost) · fx_today
            #   FX     € = shares · cost          · (fx_today − fx_cost)
            #   sum     = shares · (price·fx_today − cost·fx_cost) = total P&L €
            # fx_cost (fx_rate_cost) is set by the cloud backend; on the EUR-only
            # demo it's absent, so FX € is 0 — which is correct there.
            fx_pnl = 0.0
            asset_total_pnl = 0.0
            has_fx = False
            for _a, _p in snapshot["positions"].items():
                if _p.get("is_cash"):
                    continue
                _pnl_a = _p.get("total_return_eur")
                if _pnl_a is not None and not pd.isna(_pnl_a):
                    asset_total_pnl += _pnl_a
                _sh = _p.get("shares") or 0
                _inc = _p.get("inception_price")
                _fx_now = _p.get("fx_rate")
                _fx_cost = _p.get("fx_rate_cost")
                if (_inc is not None and _fx_now is not None
                        and _fx_cost is not None and _sh):
                    contrib = _sh * _inc * (_fx_now - _fx_cost)
                    fx_pnl += contrib
                    if abs(_fx_now - _fx_cost) > 1e-9:
                        has_fx = True
            market_pnl = asset_total_pnl - fx_pnl
            fx_share = (fx_pnl / asset_total_pnl) if abs(asset_total_pnl) > 1e-9 else 0.0

            cur_panel = st.container(border=True)
            cur_panel.markdown(
                va1theme.panel_head(
                    "Currency effect",
                    "how much of the holdings' P&L comes from FX moves, "
                    "not the markets"),
                unsafe_allow_html=True,
            )
            with cur_panel:
                if not has_fx:
                    st.caption("All holdings are in the portfolio currency — "
                               "no currency effect.")
                else:
                    g1, g2, g3 = st.columns(3)
                    g1.markdown(
                        kpi("FX gain / loss", euro(fx_pnl),
                            f"{fx_share:+.1%} of holdings P&L",
                            "up" if fx_pnl >= 0 else "down"),
                        unsafe_allow_html=True,
                    )
                    g2.markdown(
                        kpi("Market gain / loss", euro(market_pnl),
                            "price moves, FX-neutral",
                            "up" if market_pnl >= 0 else "down"),
                        unsafe_allow_html=True,
                    )
                    g3.markdown(
                        kpi("Holdings P&L", euro(asset_total_pnl),
                            "market + FX",
                            "up" if asset_total_pnl >= 0 else "down"),
                        unsafe_allow_html=True,
                    )
...
if st.session_state.get("_show_tx") and not _DEMO:
    # Dedicated Transactions view — opened by the "+ Transaction" button in
    # the app bar (moved out of History). Rendered BEFORE the tabs and ends
    # with st.stop(), so it fully replaces the dashboard while open.
    _tx_back, _tx_head = st.columns([1.4, 8.6], vertical_alignment="center")
    with _tx_back:
        if st.button("← Back", key="__tx_back", width="stretch"):
            st.session_state["_show_tx"] = False
            st.rerun()
    with _tx_head:
        st.markdown(
            va1theme.header_html("Transactions", _pf_ccy,
                                 "Record buys, sells and cash — everything "
                                 "recalculates automatically", version=""),
            unsafe_allow_html=True,
        )
    ...
    if transactions_df.empty:
        ...
        if _sells:
            ...
            st.markdown(
                va1theme.df_table(
                    sell_tbl,
                    right_cols=["Shares sold", "Sell price", "Avg cost",
                                "Realized", "Market (today)", "Realized P&L"],
                    signed_cols=["Realized", "Market (today)", "Realized P&L"]),
                unsafe_allow_html=True,
            )
    ...
    # ----------------------------------------------------------------
    with st.expander("Import several transactions from Excel or CSV", expanded=False):
...
# History no longer has a Transactions sub-tab — Transactions moved to its own
# view (the "+ Transaction" button in the app bar). History = Price history,
# rendered straight into the tab (alias, no sub-tabs).
sub_prices = tab_history
...
with tab_dashboard:
    ...
    k2.markdown(
        kpi("Unit value (base 100)", f"{vl:,.2f}", "time-weighted",
            help="Time-weighted (TWR): pure MARKET performance, base 100 at "
                 "inception. It neutralises WHEN you invested, so adding money "
                 "on different dates doesn't move it. Equals Total return when "
                 "all your capital went in at one date; differs when you invest "
                 "at different times — that gap is normal, not an error."),
        unsafe_allow_html=True,
    )
    k3.markdown(
        kpi("Total return", pct(total_return_pct), "money-weighted",
            "up" if total_return_pct >= 0 else "down",
            help="Money-weighted (MWR): your ACTUAL gain, (value − invested) / "
                 "invested. It depends on WHEN you added money. Differs from the "
                 "time-weighted unit value whenever you invest at different "
                 "dates — both are correct, they measure different things."),
        unsafe_allow_html=True,
    )
    k4.markdown(
...
with tab_settings:
        ...
        if st.button("Run diagnostic", key="diag_run") and probe_tk.strip():
            ...
            if rep.get("error") and "windows" not in rep:
                ...
                st.write(f"**Chunked oldest→newest** — stitched last date = "
                         f"**{rep['stitched_last']}**:")
                st.markdown(
                    va1theme.df_table(pd.DataFrame([
                        {"window": w["window"], "rows": w["n"],
                         "first": w["first"], "last": w["last"],
                         "secs": w["secs"], "error": w["error"] or ""}
    ...
    rows = [{"Name": p["name"],
            ...
            for p in pfs]
    st.markdown(va1theme.df_table(pd.DataFrame(rows)), unsafe_allow_html=True)
