...
def load_price_history(full: bool = False) -> pd.DataFrame:
    ...
    uid = _user_id()
    pid = current_portfolio()["id"]
    res = (_sb().table("prices").select("trade_date,asset,price")
...
def augmented_price_history(price_history: pd.DataFrame | None = None) -> pd.DataFrame:
    """Close-price history with a synthetic PURCHASE row inserted for every BUY.

    For each buy (asset, date, price) a row is added at the buy date carrying the
    EXACT price paid for that asset (other assets keep their close as-of that
    date), sorted to appear just BEFORE that day's close. Result columns:
    date, is_buy (bool), <asset>… So each asset's series reads
    [prix d'achat, 1ʳᵉ clôture, clôtures suivantes…] and every re-buy is visible.
    ...
    cols = [c for c in ph.columns if c not in ("date", "is_buy")]
    if ph.empty:
        out = ph.copy()
    ...
    for a, lst in buys.items():
        if a not in cols:
            continue
        for d, p in lst:
            by_date.setdefault(d, {})[a] = p   # same-day re-buy: last wins
    ...
    events = []   # (timestamp, is_buy, {col: val})
    for _, r in ph.iterrows():
        events.append((r["date"], False, {c: r[c] for c in cols}))
    for d, amap in by_date.items():
        vals = close_vals_asof(d)
        vals.update(amap)                       # override bought assets with buy price
        events.append((pd.Timestamp(d), True, vals))

    # date asc, and on the same date the purchase row comes before the close
    events.sort(key=lambda e: (e[0], 0 if e[1] else 1))
    rows = [{"date": ts, "is_buy": is_buy, **vals} for ts, is_buy, vals in events]
    return pd.DataFrame(rows)[["date", "is_buy", *cols]]


def load_transactions() -> pd.DataFrame:
    uid = _user_id()
    pid = current_portfolio()["id"]
    res = (_sb().table("transactions").select(
...
def _backfill_asset_since(asset: str, since: dt.date) -> int:
    """Fetch + store daily adjusted closes for `asset` from `since` to today,
    when the asset has no prices at/before `since` yet.

    Mirrors data_sqlite so a newly-added asset gets a CONTINUOUS history from
    its acquisition date — otherwise its line in the price chart starts wrong
    and it's missing from per-asset Pro views (which need a real series).
    ...
    if res.data:
        earliest = dt.date.fromisoformat(res.data[0]["trade_date"][:10])
        if earliest <= since:
            return 0
    try:
        import prices as _prices
        series = _prices.fetch_ticker_history(ticker, since, dt.date.today())
    except Exception:
        return 0
...
def _fx_load_cache_from_db(uid: str, pid: str) -> None:
    ...
    if (uid, pid) in _FX_LOADED:
        return
    res = (_sb().table("fx_rates").select("trade_date,pair,rate")
             .eq("user_id", uid).eq("portfolio_id", pid).execute())
    for r in (res.data or []):
...
def refetch_all_prices_from_inception() -> dict:
    """Wipe + refetch prices from inception (for the user's current portfolio)."""
    uid = _user_id()
    pid = current_portfolio()["id"]
    _sb().table("prices").delete().eq("user_id", uid).eq("portfolio_id", pid).execute()
    # Re-fetch via daily_update's backfill path
    import prices as _prices
    inc = get_inception_date()
    first_tx = _first_tx_date_by_asset()   # don't store prices before acquisition
    rows = []
    try:
        hist = _prices.fetch_history(inc, dt.date.today())
        for asset, series in hist.items():
            if asset not in ASSETS:
                continue
            fd = first_tx.get(asset, inc)
            for d, p in series:
                if p is not None and d >= fd:
                    rows.append((d, asset, float(p)))
    except Exception as e:
        return {"refetched": 0, "error": str(e)}
    backfill_prices(rows)
    return {"refetched": len(rows)}
