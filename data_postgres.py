...
def heal_price_gaps() -> int:
    ...
    # covers weekends/holidays), the portfolio is being kept up to date, so skip
    # the per-asset gap scan (2 DB calls/asset) for a fast load. A real tail gap
    # leaves `latest` old, which still triggers the full heal below.
    try:
        latest = latest_price_date()
        if latest is not None and (dt.date.today() - latest).days <= 4:
            return 0
    except Exception:
        pass
    added = 0
...
def refetch_all_prices_from_inception() -> dict:
    ...
    Fetches FIRST and only wipes once we actually have data to put back, so a
    failed/empty Yahoo response can't leave the table empty. Each asset keeps its
    own last adjusted close (Yahoo returns the last close per ticker)."""
    uid = _user_id()
    pid = current_portfolio()["id"]
    import prices as _prices
