...
# FX history survives cold starts.
#
...
def _fx_fetch_yahoo(from_ccy: str, to_ccy: str,
    ...
    """Daily FX history for `{from}{to}` via Yahoo, through the requests-based
    `prices._yahoo_chart` — NOT via yfinance.

    ⚠️ This is fix #1 finishing the job it started. data_sqlite._fx_fetch_yahoo
    (which this used to re-export) does `import yfinance`. yfinance is NOT
    installed on the cloud, so on Streamlit Cloud that import raised, the fetch
    returned [], and FX silently froze: any pair already in the fx_rates table
    forward-filled its last row forever (the "stuck ~10 days" rate), and any
    NEW pair fell through to fx_rate()'s 1.0 fallback (a non-EUR portfolio with
    a foreign-currency trade got NO conversion at all). Routing through
    `prices` — same cookie+crumb session the prices use — makes FX actually
    refresh on the cloud.

    Yahoo's `{FROM}{TO}=X` gives 1 FROM = X TO. If that symbol has no data we
    fetch the inverse and reciprocate, matching the previous behaviour."""
    if from_ccy == to_ccy:
        return [(start, 1.0)]
    import prices as _prices
    rows = _prices._yahoo_chart(f"{from_ccy}{to_ccy}=X", start, end)
    if rows:
        return rows
    inv = _prices._yahoo_chart(f"{to_ccy}{from_ccy}=X", start, end)
    return [(d, 1.0 / v) for d, v in inv if v] if inv else []
...
def compute_vl_series(price_history: pd.DataFrame | None = None,
    ...
    for i, d in enumerate(dates):
        ...
        if i == 0:
            # Anchor the initial units on the CAPITAL INVESTED by the inception
            # date (external deposits), NOT the inception-day close NAV. This is
            # the standard time-weighted anchoring: base 100 represents the money
            # you actually put in. Consequence — with a single lump-sum and no
            # later flows, VL base 100 equals the money-weighted Total Return
            # EXACTLY (both are total_value / invested − 1). The old nav/100
            # anchor drifted from Total Return whenever the inception close
            # differed from the execution price you entered. Falls back to nav
            # when no deposit is recorded by the first date.
            invested_0 = external_deposits_as_of(dates[0])
            base0 = invested_0 if invested_0 > 0 else nav
            units = (base0 / 100.0) if base0 > 0 else 1.0
        else:
            cf = cf_on.get(d, 0.0)
            if cf and units:
    ...
    if not price_history.empty:
        ...
        for asset in ASSETS:
            ...
            if cur is not None and inc_price:
                ...
                # Additive field for the FX P&L decomposition in app.py. FX rate
                # at the cost-basis date, alongside p["fx_rate"] (today). Lets
                # the UI split total_return_eur into a market part and a currency
                # part without recomputing the cost basis. No existing number
                # depends on it. (data_sqlite doesn't set it; the demo is
                # EUR-only so its FX P&L is genuinely 0.)
                p["fx_rate_cost"] = r_inc
            else:
                p["total_return"] = 0.0
