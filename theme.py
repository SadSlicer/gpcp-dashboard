...
# Light was washed out: near-white cards on a near-white ground, hairline
# borders, and pale muted text (#8496AC ≈ 3:1) all below the contrast the dark
# theme has. Deepened the ground so white cards separate, darkened the muted /
# secondary text past WCAG AA (4.5:1) on white, and strengthened borders and
# chart gridlines. Numbers checked against #FFFFFF / #EEF2F7.
LIGHT = Tokens(
    BG_DEEP        = "#EAEEF4",   # deeper ground so white surfaces read as raised
    BG_BASE        = "#F4F7FA",
    BG_ELEVATED    = "#FFFFFF",
    BG_INPUT       = "#FFFFFF",
    SURFACE_1      = "#FFFFFF",
    SURFACE_2      = "#F1F5FA",
    SURFACE_HOVER  = "#E9EFF6",
    BORDER_1       = "#CDD6E2",   # visible hairline (was #E2E8F0, near-invisible)
    BORDER_2       = "#AEBACB",
    BORDER_3       = "#7E8DA3",
    TEXT_PRIMARY   = "#0B1220",   # 16.6:1 on white
    TEXT_SECONDARY = "#3A4658",   # 8.6:1 — body / sub text
    TEXT_MUTED     = "#586475",   # 5.7:1 — labels, captions (was 3:1)
    TEXT_FAINT     = "#8B98A9",   # 3.1:1 — decorative only
    TEXT_DISABLED  = "#B4BECC",
    ACCENT         = "#9A5B00",   # gold, 4.6:1 on white
    ACCENT_SOFT    = "#B8730C",
    ACCENT_DEEP    = "#7A4700",
    ACCENT_TINT    = "#FBF1DE",
    ACCENT_BORDER  = "#D9BE86",
    ACCENT_ON      = "#FFFFFF",
    BRAND          = "#0F2547",
    BRAND_ON       = "#FFFFFF",
    DANGER         = "#B3261E",
    DANGER_TINT    = "#FBEAE8",
    SUCCESS        = "#0B6B62",   # 4.9:1 on white (was #0F766E ≈ 4.3:1)
    SUCCESS_TINT   = "#E1F0ED",
    INFO           = "#0369A1",
    GRID           = "#DBE2EC",   # visible gridlines (was #EDF1F6)
)
...
# Vertical left nav (1st-level st.tabs) — emitted ONLY when build_css is called
# with sidebar_nav=True (the app), never on the login screen. Plain CSS string
# (single braces, CSS variables) so it drops straight into the outer f-string.
# Version-specific like fix #6 (Streamlit pinned 1.57.0); `:has()` targets the
# list wrapper structurally without a fragile :nth-child.
_NAV_CSS = """
  /* ---- 1st-level nav → THIN vertical rail on the left --------------- */
  .stTabs > div { display: flex; align-items: flex-start; gap: 18px; }
  .stTabs > div > div:has(> [data-baseweb="tab-list"]),
  .stTabs > div > div:has(> [role="tablist"]) {
    flex: 0 0 116px; position: sticky; top: 8px; align-self: stretch;
  }
  .stTabs > div > [data-baseweb="tab-panel"],
  .stTabs > div > [role="tabpanel"] { flex: 1 1 auto; min-width: 0; }
  .stTabs > div > [data-baseweb="tab-border"] { display: none; }
  /* Hide BaseWeb's sliding highlight bar — active state is shown by the tab's
     own pill background/border, so the bar just leaks as a stray line. */
  .stTabs [data-baseweb="tab-highlight"] { display: none !important; }
  .stTabs [data-baseweb="tab-list"], .stTabs [role="tablist"] {
    flex-direction: column; align-items: stretch;
    gap: 2px; padding: 0 9px 0 0;
  ...
  .stTabs [data-baseweb="tab"], .stTabs [role="tab"] {
    ...
    min-height: 32px !important; letter-spacing: 0 !important;
  }

  /* ---- 2nd-level sub-tabs → ALSO vertical, but SMALLER ---------------
     Nested st.tabs inherit the vertical layout above; here we only make
     them a thinner, quieter secondary rail so they read as sub-navigation,
     not another top-level menu. More specific (tab-panel ancestor) → wins. */
  [data-baseweb="tab-panel"] .stTabs > div,
  [role="tabpanel"] .stTabs > div { gap: 14px; }
  [data-baseweb="tab-panel"] .stTabs > div > div:has(> [data-baseweb="tab-list"]),
  [data-baseweb="tab-panel"] .stTabs > div > div:has(> [role="tablist"]),
  [role="tabpanel"] .stTabs > div > div:has(> [role="tablist"]) {
    flex: 0 0 104px;
  }
  [data-baseweb="tab-panel"] .stTabs [data-baseweb="tab-list"],
  [data-baseweb="tab-panel"] .stTabs [role="tablist"],
  [role="tabpanel"] .stTabs [role="tablist"] {
    min-height: 0; padding: 0 7px 0 0;
  }
  [data-baseweb="tab-panel"] .stTabs [data-baseweb="tab"],
  [data-baseweb="tab-panel"] .stTabs [role="tab"],
  [role="tabpanel"] .stTabs [role="tab"] {
    padding: 4px 7px !important; font-size: 10.5px !important;
    min-height: 24px !important;
  }

  /* ---- narrow screens: fold both levels back to horizontal ---------- */
  @media (max-width: 900px) {
    .stTabs > div { display: block; gap: 0; }
    .stTabs > div > div:has(> [data-baseweb="tab-list"]),
    .stTabs > div > div:has(> [role="tablist"]) { flex: none; position: static; }
    .stTabs [data-baseweb="tab-list"], .stTabs [role="tablist"] {
      flex-direction: row; min-height: 0; border-right: none;
      border-bottom: 1px solid var(--border-1); padding: 0 0 8px 0;
      overflow-x: auto; flex-wrap: nowrap;
    }
    .stTabs [data-baseweb="tab"], .stTabs [role="tab"] {
      width: auto; min-height: 40px !important;
    }
  }
"""


def build_css(active: str | None = None, *, sidebar_nav: bool = False) -> str:
    """The stylesheet for the ACTIVE palette (one per render).

    `sidebar_nav` turns the 1st-level st.tabs into the vertical left nav. It is
    OFF by default so the login screen (auth.py), which also uses st.tabs for
    "Sign in / Create an account", keeps its normal horizontal tabs — the app
    (app.py) opts in with sidebar_nav=True.
    ...
    nav_css = _NAV_CSS if sidebar_nav else ""
...
{palette}
  ...
  /* ============================================================
     SECTION HEAD — styled as a panel header bar so every tab echoes
     the Dashboard's panel look (accent left edge, ruled surface),
     without wrapping each block in a container.
     ============================================================ */
  .va1-section-head {{
  ...
  .va1-section-sub {{ font-size: 11.5px; color: var(--text-muted); font-weight: 400; }}
  /* The divider becomes the bottom edge that closes the header bar and
     carries into the content below it. */
  .va1-divider {{
  ...
  .kpi-card:hover .kpi-hint {{ color: var(--text-secondary); border-color: var(--border-3); }}
  .kpi-badge-live {{
...
def df_table(df, *, fmt: dict | None = None,
             signed_cols: list[str] | None = None,
             right_cols: list[str] | None = None,
             key_first: bool = True) -> str:
    """Render a DataFrame as the app's HTML table — the one-liner replacement
    for `st.dataframe(styler)` so every table matches the Holdings look.

    `fmt` maps column → a format string ("{:,.2f} €") or a callable, applied
    per cell (NaN/None → "—"). `signed_cols` are tinted green/red by the raw
    value's sign. Numeric columns (in `fmt`, numeric dtype, or `right_cols`)
    are right-aligned; the first column reads as the row key.
    """
    import pandas as _pd
    from html import escape as _escape

    fmt = fmt or {}
    signed = set(signed_cols or [])
    headers = [str(c) for c in df.columns]

    if right_cols is not None:
        right = {df.columns[i] for i, c in enumerate(df.columns) if c in set(right_cols)}
    else:
        right = {c for c in df.columns
                 if c in fmt or _pd.api.types.is_numeric_dtype(df[c])}
    num_idx = {i for i, c in enumerate(df.columns) if c in right}

    def _fmt_cell(col, v) -> str:
    ...
    for _, r in df.iterrows():
        ...
        for i, col in enumerate(df.columns):
            v = r[col]
            text = _fmt_cell(col, v)
            tone = "key" if (i == 0 and key_first) else ""
            if col in signed:
                # Tone by sign — works whether the cell is a raw number or an
                # already-formatted string ("+1.23 %").
                if isinstance(v, (int, float)) and not _pd.isna(v):
                    tone = "pos" if v > 0 else ("neg" if v < 0 else "")
                elif isinstance(text, str) and text not in ("—", ""):
                    if text.lstrip().startswith("+"):
                        tone = "pos"
                    elif text.lstrip().startswith(("-", "−")):
                        tone = "neg"
            cls = " ".join(c for c in (("num" if i in num_idx else ""), tone) if c)
            tds.append(f'<td class="{cls}">{_escape(text)}</td>')
        body.append(f"<tr>{''.join(tds)}</tr>")

    head = "".join(
        f'<th class="{"num" if i in num_idx else ""}">{_escape(h)}</th>'
        for i, h in enumerate(headers)
    )
    return (
        '<div class="gp-tbl-wrap"><table class="gp-tbl">'
...
def kpi_card(label: str, value: str,
    ...
    # `help` becomes a native hover tooltip on the whole card plus a small "?"
    # cue on the label — used to explain metrics that are correct but easy to
    # misread (e.g. why time-weighted VL differs from money-weighted return).
    title_attr = f' title="{escape(help)}"' if help else ""
    hint = ('<span class="kpi-hint" aria-hidden="true">?</span>') if help else ""
    return (
        f'<div class="kpi-card"{title_attr}>'
        f'<div class="kpi-label"><span>{label}{hint}</span>{badge}</div>'
