...
<style>
  ...
  /* Global loading spinner — a small floating card with a crisp spinning ring,
     shown while a rerun is in progress. NO full-page dim (page stays bright).
     IMPORTANT: it uses its OWN element (.gpcp-loader, injected once in app.py)
     and that element's ::after — NOT body::before/::after, which the animated
     background already owns (those carry filter: blur() + blend, which is what
     made the ring fuzzy). The status widget stays in the DOM while a rerun runs,
     so :has() reveals the loader. No-op on browsers without :has() — graceful. */
  .gpcp-loader {{
    display: none;
    position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
    z-index: 99999; width: 86px; height: 86px;
  ...
  .gpcp-loader::after {{
    ...
    animation: va-load-spin 0.7s linear infinite;
    will-change: transform;
  }}
  @keyframes va-load-spin {{ to {{ transform: rotate(360deg); }} }}
